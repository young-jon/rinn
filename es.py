...
### JON YOUNG
def compute_weight_decay_l1(weight_decay, model_param_list):
  model_param_grid = np.array(model_param_list)
...
class SimpleGA:
  ...
  def __init__(self, num_params,      # number of model parameters
               ...
               weight_decay=0.01,     # weight decay coefficient
               weight_decay_l1=0.01   ### JON YOUNG
              ):
  ...
  def ask(self):
    '''
    returns a list of parameters. epsilon represents amount to mutate params. this is how it is used:
    child_params + self.epsilon[i]
    '''
  ...
  def tell(self, reward_table_result):
    ...
    if self.weight_decay_l1 > 0:
      l1_decay = compute_weight_decay_l1(self.weight_decay_l1, self.solutions)
      reward_table += l1_decay
...
### Jon Young 7_9_19
class ConstrainedSimpleGA:
  '''Constrained Simple Genetic Algorithm. Instead of sampling mutation noise from normal distribution,
  this class constrains the evolved parameters to a list of values. the list of values must be passed
  to the constructor.'''
  def __init__(self, num_params,      # number of model parameters
               sigma_init=0.1,        # initial standard deviation
               sigma_decay=0.999,     # anneal standard deviation
               ...
               weight_decay=0.01,     # weight decay coefficient
               weight_decay_l1=0.01,   ### JON YOUNG
               legal_param_values = [0,1], ### JON YOUNG
               mutation_rate = 0.01    ### JON YOUNG
               ):
    ...
    self.legal_param_values = legal_param_values
    self.param_indexes = range(self.num_params)
    self.num_mutated_params = round(self.num_params * self.mutation_rate)
  ...
  def ask(self):
    ...
    child_params + self.epsilon[i]
    '''
    #print('here',self.elite_params.shape, self.elite_params[1][0:10])
    #self.epsilon = np.random.randn(self.popsize, self.num_params) * self.sigma  ### small random numbers, old mutate
    
    solutions = []
    
    def mate(a, b):
      c = np.copy(a)
      idx = np.where(np.random.rand((c.size)) > 0.5)  ### randomly choose locations in weight vector ~ 50% of locations
      ### and change there values to the values in b
      c[idx] = b[idx]
      return c
    ...
    for i in range(self.popsize):
      ...
      ### mutate child_params
      # if i == 1:
      #   print('1',child_params[0:15])
      child_params[np.random.choice(self.param_indexes,self.num_mutated_params,replace=False)] = np.random.choice(self.legal_param_values,self.num_mutated_params)
      solutions.append(child_params)
      # if i == 1:
      #   print('2',child_params[0:15])

      #solutions.append(child_params + self.epsilon[i])  ### mutate all values a small amount
